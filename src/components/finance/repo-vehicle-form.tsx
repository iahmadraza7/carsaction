...
import { BODY_TYPES, humanizeEnum } from "@/lib/listing-options";
import {
  repoVehicleSchema,
  type RepoVehicleFormValues,
  emptyRepoVehicleForm,
...
type Props = {
  mode: "create" | "edit";
  vehicleId?: string;
  defaultValues?: RepoVehicleFormValues;
  initialImages?: string[];
};

export function RepoVehicleForm({
  mode,
  vehicleId,
  defaultValues,
  initialImages = [],
  ...
  } = useForm<RepoVehicleFormValues>({
    defaultValues: defaultValues ?? emptyRepoVehicleForm,
  });
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      const res = await fetch("/api/finance/uploads", { method: "POST", body });
  ...
  async function onSubmit(values: RepoVehicleFormValues) {
    const parsed = repoVehicleSchema.safeParse({ ...values, images });
    if (!parsed.success) {
      ...
      for (const [field, messages] of Object.entries(fieldErrors)) {
        if (field === "images") {
          ...
          setError(field as keyof RepoVehicleFormValues, { message: messages?.[0] });
        }
      }
      return;
    }
    ...
    const url =
      mode === "create"
        ...
        : `/api/finance/vehicles/${vehicleId}`;
    ...
    if (!res.ok) {
      if (data?.errors) {
        for (const [field, messages] of Object.entries(
          ...
          else setError(field as keyof RepoVehicleFormValues, { message: messages?.[0] });
        }
      } else {
        toast.error(data?.error ?? "Could not save the vehicle. Please try again.");
      }
      return;
    }

    toast.success(mode === "create" ? "Repo vehicle posted." : "Vehicle updated.");
    router.push("/finance/dashboard");
    router.refresh();
  }

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid max-w-2xl gap-8" noValidate>
      ...
      <Section title="Vehicle basics">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Body type" error={errors.bodyType?.message}>
            <select className={controlClass} {...register("bodyType")}>
              {BODY_TYPES.map((t) => (
          ...
          <Field label="Condition (optional)" error={errors.condition?.message}>
            <input
              className={controlClass}
          ...
          <Field label="Viewing location (optional)" error={errors.location?.message}>
            <input
              className={controlClass}
              placeholder="Ubi yard"
              {...register("location")}
            />
          </Field>
          <Field className="sm:col-span-2" label="Description (optional)" error={errors.description?.message}>
            <textarea
              className={`${controlClass} h-28 py-2`}
              placeholder="Additional notes for dealers"
      ...
      <Section
        ...
        hint="Dealers can bid between these times. Close must be after open and in the future."
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Bidding opens" error={errors.biddingOpensAt?.message}>
            <input
              className={controlClass}
              type="datetime-local"
              {...register("biddingOpensAt")}
            />
          </Field>
          <Field label="Bidding closes" error={errors.biddingClosesAt?.message}>
            <input
              className={controlClass}
              type="datetime-local"
          ...
          {/* CONFIDENTIAL: reserve price — finance owner / admin only. Never shown to dealers. */}
          <Field
            label="Reserve price (optional, confidential)"
          ...
          >
            <input
              ...
              type="number"
              inputMode="numeric"
              placeholder="Not shown to dealers"
              {...register("reservePrice")}
            />
            <p className="text-xs text-muted-foreground">
              Only your company can see this. It is never shown on the public auction pages.
            </p>
          </Field>
        </div>
      </Section>
